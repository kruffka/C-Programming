...
<br>
Привет, меня зовут Вейлер Андрей Игоревич и тут я буду оставлять свои лекции и примеры по Программированию для 1 курса        
...
- Успеваемость в гугл табличке. Там будет самая актуальная инфа о ваших лабах. Раз в пару недель будет общий рейтинг 
- Лекции. Презентации и ссылки
- Лабы, которые нужно сдать за семестр
- Список литературы        

У кого свои ноутбуки ставим Linux (если у вас не MacOS) проще всего будет WSL (или Ubuntu через Microsoft Store), чуть подробнее в презентации. Ноутбук - сильно упростит вам жизнь, достаточно САМОГО простого для ***учебы***            

**Важно: Посещаешь занятия = ты молодец**
...
- Мессенджеры, браузеры, сайты в них, ИИ для генерации видео и т.д... тонны кода
...
Простыми словами эта штука нужна компилятору для понимания кто такой `printf()`       
...
**ВАЖНО**: 
- Если не написать '-o hello', то на выходе получим исполняемый файл с именем 'a.out' - такое имя по умолчанию дает компилятор.     
- Так не делать: 'gcc hello.c -o hello.c' - вы затрете свой исходный файл исполняемым
...
Представим себе **коробку**, в которую можно класть яблоки. Возьмем маркер и напишем на коробке - "коробка яблок". Можно взять эту "коробка яблок" и накидать в нее яблок (записать что-то в переменную), а потом взять и посмотреть сколько там сейчас всего яблок (прочитать из переменной). 

При этом коробка **не бесконечная**, в нее влезет определенное количество яблок. Можно создать коробку с гречкой, в нее яблоки складывать не будем, т.к. эти коробки предназначены для разных типов продуктов         
...
Попробуем запустить [print_vars.c](src/print_vars.c)
```c
#include <stdio.h>
...
Допустим нам предлагают 2147483648$ или удвоить и отдать следующему, мы соглашаемся удвоить и передать следующему [overflow.c](src/overflow.c)                  
```c
...
Например для char мы получаем не -128..127, а 0..255        
...
Для тех кто еще читает смотрим книгу от создателя языка Си. Также можно посмотреть видосы либо попросить ИИ вас поучить (главное - учить, а не списать), ИИ стараемся использовать как **инструмент для обучения** если хотим развития.                      

Язык СИ
- Brian W. Kernighan. Dennis M. Ritchie - The C programming language, 1988: github_pdf
-CS50 или CS50x - Гарвадский курс каждый год: Youtube
- Т. Хирьянов - Курс молодого бойца СИ: YouTube
- К. Владимиров - Практика С для МФТИ: YouTube


На досуге
- Для начинающих: Stepik, W3Schools и другие онлайн штучки
- Для продолжающих/олимпиадное/собесы: leetcode
