...
**Циклы (loops)**
- Позволяют выполнять инструкцию столько раз сколько попросим         
- Сокращают количество кода (Делают код более читаемым)         
...
Давайте попробуем **три** раза мяукнуть на экран (полный пример [while.c](src/while.c)):
...
<img src="img/uia.webp" width="200"/><img src="img/cat.webp" width="200"/>                         
...
**Когда может понадобиться?** Когда в любом случае нужно выполнить код **хотя бы один раз**, полезно бывает когда хочется сначала сделать, а потом проверить что сделали, например в [do_while.c](src/do_while.c) мы просим пользователя ввести положительное число, а затем проверяем ввод пользователя на корректность.             
...
2. Проверяем условие i < 5           
   1.   Выводим значение i и пробел            
   2.  Прибавляем 1 к i (шаг 1)        
...
Пример [break.c](src/break.c) с циклом while:        
```c
int i = 0;
while (i < 10) {
    printf("i = %d\n", i);
...
Обычно его используют для пропуска **итерации цикла**, например [continue.c](src/continue.c) и цикл for:         
...
**Nested (вложенный)** - в программировании можно использовать циклы внутри циклов, так например, мы можем вывести таблицу умножения (3x3) [nested.c](src/nested.c):
...
##  4. <a name='4'></a>Examples

Все примеры можно [найти тут](src/)                 
            
Рассмотрим пару примеров с циклами, например поищем является ли число **простым** (делится только на себя и на 1).            

Рассмотрим [prime.c](src/prime.c)              

**Самый простой вариант**, чтобы понять является число простым или нет - это взять это число и делить его на все подряд, начиная с 2 и до самого числа - 1. Например, для 5 мы будем брать 5 и делить по очереди на 2, на 3 и на 4. Если хоть на что-то поделилось - число **не простое (составное)**.                  

Для этого объявим переменную i = 2 в цикле, и i будем менять с шагом 1 до нашего числа a не включая само число (строгий знак меньше). С помощью условия проверяем есть ли остаток от деления. Если **ХОТЯ бы раз** поделилось без остатка, то мы сразу понимаем что число не может быть простым - выходим из цикла через break, таким образом наша i не достигнет последней итерации i == a. Если цикл дойдет до конца, то i станет = a, что говорит о том, что не нашлось такой i в диапазоне [2, a-1], которое является делителем числа a.             

Здесь важно переменную i объявить за циклом! При объявлении в цикле в блоке инициализации for, переменные будут видны лишь внутри цикла. Компилятор сразу выдаст об ошибке если мы перенесем int i в цикл for, типа такого:        
