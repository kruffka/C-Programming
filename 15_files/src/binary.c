...
int main(int argc, char *argv[]) {
    ...
    for (int i = 0; i < N; i++) {
        ...
        size_t written = fwrite(&array[i], sizeof(struct some_data), 1, file);
        if (written != 1) {
            printf("Error writing element %d\n", i);
            fclose(file);
            exit(EXIT_FAILURE);
        }        
    ...
    size_t elements_read = fread(array, sizeof(struct some_data), N, file);
    if (elements_read != N) {
        if (feof(file)) {
            printf("Warning: Unexpected end of file. Read %zu elements instead of %d\n", 
                   elements_read, N);
        } else if (ferror(file)) {
