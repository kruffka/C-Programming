...
Пару определений:
**Файлы** - представляют собой именованные области
внешней (дисковой) памяти, с которыми программы могут обмениваться
информацией.      

**Поток** - это логический канал, предназначенный для выполнения операций
ввода/вывода. Каждому файлу при его открытии ставится в соответствие
поток.     
...
<img src="img/fractal.png" width ="400">        
А теперь провалимся глубже в ОС и постараемся понять
...
<img src="./img/big_thumb_cat.png" width ="500">          
...
В си обработка файлов делится на 2 типа - текстовый и бинарный. Хотя по факту для ОС это все одно и то же. Разница есть только для самих приложений.                 
...
```
- Считывает всю строку (не больше size - 1) из файла в str
...
```
- Печатает всю строку в файл (без форматирования как fprintf)
...
```
- Считывает указанные байты данных из файла. Возвращает количество успешно считанных элементов (должно быть равно nmemb)
