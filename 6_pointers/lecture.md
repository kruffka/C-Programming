...
Все это похоже на один очеееень длинный массив байт, где индексы этого массива - **есть адреса** (строка с Address), а значения массива - **значения, что хранятся в памяти** (строка Value)                          
...
**Указатель (pointer)** - это **переменная**, которая хранит **адрес ячейки в памяти**.      
...
```
Такая штука называется **разыменовыванием (dereference)** - когда мы переходим по адресу, что хранит переменная-указатель и **считываем оттуда значение**. Как раз таки здесь нам и нужно знать на что мы указываем, здесь мы указываем на тип char, а тип char как помним занимает ровно 1 байт, эта информация нам необходима для того чтобы понимать СКОЛЬКО данных мы хотим вычитать после разыменования           
...
При объявлении указателя есть **ТОЛЬКО ДВА** варианта где поставите звездочку:                 
- тут 
  - int *ptr; 
- или тут
  - int* ptr;
...
#define NULL ((void *)0)
```
NULL - это адрес на нулевую ячейку к памяти. Всем программам к ней доступ закрыт и при попытке разыменовать NULL - вы сразу же получите segfault.                          
...
Если не сделать free после malloc, то мы получим **утечку памяти**. **Утечка памяти** - ситуация, когда программа выделяет память и не освобождает, например, можно сделать так:
```c
...
В случае если ваша програма завершилась, а вы не сделали free - то ничего страшного, **ОС подчистит всю память за вас**.         
...
int *arr = (int *)malloc(3 * sizeof(int));           
if (arr == NULL) { /* ошибка */ }
memset(arr, 0, 3 * sizeof(int)); // установить, начиная с адреса arr 12 нулей
```
...
**Самостоятельно:** В примере чему будет равна переменная a в разных областях видимости? 
```c
#include <stdio.h>
