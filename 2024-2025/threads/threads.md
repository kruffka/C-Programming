...
####  1.4.1. <a name='ThreadPool'></a>Thread Pool
[TODO] Здесь будет пример thread pool

####  1.4.2. <a name='FIFO'></a>FIFO
[TODO] Здесь будет пример FIFO с условными переменными
...
Для C/C++ есть ThreadSanitizer - это как ASan, что с памятью и утечками, только по потокам (TSan называется). Для других ЯП также можно найти подобные - просто гуглим их
