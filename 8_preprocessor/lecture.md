...
Содержимое этих заголовочных файлов полностью копируется на первом этапе компиляции (обработка препропроцессора)              

Увидеть результат обработки препроцессора можно опцией -E: 
```bash
gcc test.c -o -test.i -E 
```
где открыв файл test.i в любом текст. редакторе увидим вначале содержимое файлов, что подключили через include                        
