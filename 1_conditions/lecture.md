...
Поэтому достаточно будет следующего ([scanf_int.c](src/scanf_int.c)):             
```c
int a;
printf("type a:");
scanf("%d", &a);
...
Исходный код с примером попробовать [type_cast.c](src/type_cast.c)
...
**condition** здесь - некоторое условие, например a > 5 ([пример if.c](src/if.c)):
```c
if (a > 5) {
    printf("a > 5!\n");
}
...
Пример ([if_else.c](src/if_else.c)):
```c
...
[Полный пример if_else_if.c](src/if_else_if.c)
...
[Пример с проваливанием вниз switch.c](src/switch.c)                    
```c
    switch (x) {
