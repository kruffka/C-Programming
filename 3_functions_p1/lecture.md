...
[Полный пример с функцией суммы 2 чисел](src/sum.c)                 
...
Попробуем написать функцию поиска максимального среди двух целых чисел [max.c](src/max.c)                   
...
Напишем свою функцию (предикат), что возвращает булево значение - проверка на четность [even.c](src/even.c):      
```c
int isEven(int a) {
    return a % 2 == 0 ? 1 : 0;
}
