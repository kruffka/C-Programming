...
- В фигурных скобках {} - тело функции. Это и есть блок кода с инструкциями;         
...
```
`int sum(int a, int b)` - **заголовок** функции (**прототип** еще называют). Сама функция называется `sum`, возвращает тип данных int, а на вход принимает два int аргумента - a и b            
...
Как вызвать этот блок кода? Для **вызова функций** мы используем имя этой функции.              
...
**Самостоятельно:** Написать простой калькулятор для двух чисел и знака операции, введенных с клавиатуры. Каждая операция `+`, `-`, `*`, `/` и т.д. - отдельная функция. Используйте **switch-case** для выбора функции. **Подсказка** по вводу с клавиатуры:            
```c
int a, b;
char s; // знак
...
Компилятор проверяет функции сверху вниз в нашем исходном файле и если он найдет вызов функции раньше чем объявлена сама функция, то это может в некоторых случаях привести к неопределенному поведению. Компилятор о таких случаях денлает **предупреждения** (**warning**) - **implicit declaration** (неявное объявление)        
...
Для функций красиво делаем так:
...
Хороший тон - называть функции глаголами. Например:
